...
class EgoWholeBody_Test(HumanDataset):
    def __init__(self,
                 transform,
                 data_split,
                 data_dir='/media/cv1/SeagateHDD2TB/human_data_train',
                 id_roster=None,                
                 return_id_onehot=False,         
                 use_cache=False):
        super(EgoWholeBody_Test, self).__init__(transform, data_split)
        ...
        self.img_root = "/media/cv1/SeagateHDD2TB/EgoWholeMocap"   
        ...
        for i in idxs:
            full_img   = abs_image_paths[i]
            bbox       = self.bbox_xywh[i]
            kp2d       = self.keypoints2d[i]
            kp2d_mask  = self.keypoints2d_mask[i]
            kp3d       = self.keypoints3d[i]
            kp3d_mask  = self.keypoints3d_mask[i]
            smplx_d    = self.smplx_params[i]
            sid        = id_str_list[i]
