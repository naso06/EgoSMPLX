    ...
    """
        ...
        class RSDE(self.__class__):
            ...
            def sde(self, x, t, condition=None, mask=None, guide=False):
                ...
                score = score_fn(x, t, condition, mask)
                drift = drift - diffusion[:, None] ** 2 * score * (0.5 if self.probability_flow else 1.)
                # Set the diffusion function to zero for ODEs.
                diffusion = torch.zeros(1, device=drift.device) if self.probability_flow else diffusion
                if guide == False:
            ...
            def discretize(self, x, t, condition=None, mask=None):
                ...
                rev_f = f - G[:, None] ** 2 * score_fn(x, t, condition, mask) * (
                    1.0 if self.probability_flow else 1.)
...
class VPSDE(SDE):
    ...
    def return_alpha_sigma(self, t):
        log_mean_coeff = -0.25 * t ** 2 * (self.beta_1 - self.beta_0) - 0.5 * t * self.beta_0
        alpha = torch.exp(log_mean_coeff[:, None])
...
class subVPSDE(SDE):
    ...
    def return_alpha_sigma(self, t):
        log_mean_coeff = -0.25 * t ** 2 * (self.beta_1 - self.beta_0) - 0.5 * t * self.beta_0
        alpha = torch.exp(log_mean_coeff[:, None])
        sigma = 1. - torch.exp(2. * log_mean_coeff)
        return alpha, sigma
...
class VESDE(SDE):
    ...
    def prior_logp(self, z):
        shape = z.shape
        N = np.prod(shape[1:])
        return -N / 2. * np.log(2 * np.pi * self.sigma_max ** 2) - torch.sum(z ** 2, dim=(1)) / (
                    2 * self.sigma_max ** 2)
