...
from lib.dataset.face import FlameDataModule, N_POSES
from lib.dataset.utils import CombinedNormalizer


def parse_args(argv):
    parser = argparse.ArgumentParser(description='train diffusion model of face expressions')
    parser.add_argument('--config-path', '-c', type=str,
                        default='configs.face.subvp.timefc.get_config',
    ...
    parser.add_argument('--data-root', type=str, default='./face_data', help='dataset root')
...
class DPoserTrainer(pl.LightningModule):
    def __init__(self, config,
        ...
        self.POSE_DIM = 1  # multiply 3 (jaw) + 100 (expression)
        if config.data.rot_rep == 'rot6d':
            N_POSES += 3
        self.model = create_model(config.model, N_POSES, self.POSE_DIM)
        self.model_ema = None
        self.body_model_vis = FLAME(model_path=self.bodymodel_path, num_expressions=100, batch_size=50)
        self.body_model_eval = FLAME(model_path=self.bodymodel_path, num_expressions=100, batch_size=250)
    ...
    def setup_step_fn(self, config):
        ...
        if config.training.auxiliary_loss:
            ...
            aux_params = {'denormalize': self.denormalize_fn, 'body_model': body_model_train,
                          'model_type': "face", 'denoise_steps': config.training.denoise_steps}
    ...
    def training_step(self, batch, batch_idx):
        face_params = torch.cat([batch['jaw_pose'], batch['expression']], dim=1)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        body_out = self.body_model_eval(face_params=face_params)
    ...
    def process_and_log_meshes(self, poses, bg_img, focal, princpt, tag_prefix):
        body_out = self.body_model_vis(face_params=poses)
