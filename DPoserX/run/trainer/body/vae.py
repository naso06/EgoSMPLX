...
from lib.algorithms.vposer.model import VPoser
...
def parse_args(argv):
    ...
    parser.add_argument('--config-path', '-c', type=str,
                        ...
                        help='config files to build VPoser')
...
class VPoserTrainer(LightningModule):
    def __init__(self, config,
        ...
        self.model = VPoser(config.model, N_POSES, self.POSE_DIM)
    ...
    def training_step(self, batch, batch_idx):
        dict_reconstruction = self.model(batch['poses'])
    ...
    def _compute_loss(self, original_data, reconstructed_data):
        ...
        # Reconstruction loss
        with torch.no_grad():
            bm_orig = self.bm_train(body_pose=original_data['poses'])
...
def main(args, config, try_resume=False):
    ...
    model = VPoserTrainer(config, args.bodymodel_path, data_path, N_POSES,
                          train_loader=data_module.train_dataloader(),
                          val_loader=data_module.val_dataloader(), )

    logger = TensorBoardLogger(f"logs/vposer/{config.dataset}", name=args.name)
    model_logger = ModelSizeCallback()
    time_monitor = TimerCallback()
    lr_monitor = LearningRateMonitor()

    ckpt_dir = f"checkpoints/vposer/{config.dataset}/{args.name}"
