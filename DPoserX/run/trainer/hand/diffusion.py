...
from lib.dataset.hand import ManoDataModule, N_POSES
from lib.dataset.utils import Posenormalizer
from lib.dataset.hand.evaler import Evaler


def parse_args(argv):
    parser = argparse.ArgumentParser(description='train diffusion model of single hand pose')
...
class DPoserTrainer(pl.LightningModule):
    def __init__(self, config,
        ...
        self.model_ema = None
        self.body_model_vis = MANO(model_path=self.bodymodel_path,
                                   is_rhand=True,
                                   batch_size=50, )
        self.body_model_eval = MANO(model_path=self.bodymodel_path,
                                    is_rhand=True,
                                    batch_size=250)
    ...
    def setup_step_fn(self, config):
        ...
        if config.training.auxiliary_loss:
            body_model_train = MANO(model_path=self.bodymodel_path,
                                    is_rhand=True,
            ...
            aux_params = {'denormalize': self.denormalize_fn, 'body_model': body_model_train,
                          'model_type': "hand", 'denoise_steps': config.training.denoise_steps}
    ...
    def training_step(self, batch, batch_idx):
        poses = self.normalize_fn(batch['hand_pose'], from_axis=True)
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        hand_pose = self.denormalize_fn(all_results, to_axis=True)
        body_out = self.body_model_eval(hand_pose=hand_pose)
    ...
    def process_validation_batch(self, poses):
        ...
        '''     ******* task2 completion *******     '''
        mask, observation = create_mask(poses, part='pinky_ring_finger', model='hand')
        ...
        evaler = Evaler(hand_model=self.body_model_vis, part='pinky_ring_finger')
        eval_results = evaler.multi_eval_hands(preds, gts)
    ...
    def process_and_log_meshes(self, poses, bg_img, focal, princpt, tag_prefix):
        body_out = self.body_model_vis(hand_pose=poses)
