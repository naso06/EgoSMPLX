...
from lib.dataset.whole_body import SmplxDataModule, POSES_LIST, SmplxMixedDataModule
from lib.dataset.utils import CombinedNormalizer
from lib.dataset.whole_body.evaler import Evaler

def parse_args(argv):
    parser = argparse.ArgumentParser(description='train diffusion model of whole-body pose')
    parser.add_argument('--config-path', '-c', type=str,
                        default='configs.wholebody.subvp.mixed.get_config',
    ...
    parser.add_argument('--data-path', type=str, default='./', help='normalizer folder')
    parser.add_argument('--data-root', type=str, default='./wholebody_data', help='dataset root')
...
class DPoserTrainer(pl.LightningModule):
    def __init__(self, config,
        ...
        self.DATA_DIM = POSES_LIST[0] * self.POSE_DIM + 2 * POSES_LIST[1] * self.POSE_DIM + POSES_LIST[2]  # two hands
        self.model = create_wholebody_model(config.model, POSES_LIST, self.POSE_DIM)
        self.model_ema = None
        ...
        self.sampling_shape = (config.eval.batch_size, self.DATA_DIM)
    ...
    def setup(self, stage=None):
        if stage == 'fit':
            ...
            Normalizer = CombinedNormalizer(
                data_path_dict=self.data_path, model='whole-body',
                normalize=self.config.data.normalize, min_max=self.config.data.min_max, rot_rep=self.config.data.rot_rep,
                device=self.device)
    ...
    def setup_step_fn(self, config):
        ...
        if config.training.auxiliary_loss:
            ...
            aux_params = {'denormalize': self.denormalize_fn, 'body_model': body_model_train,
                          'model_type': "whole-body", 'denoise_steps': config.training.denoise_steps}
            kwargs.update(aux_params)
        if getattr(config.training, 'random_mask', False):
            mask_params = {'random_mask': True,
                           'min_mask_rate': config.training.min_mask_rate,
                           'max_mask_rate': config.training.max_mask_rate,
                           'observation_type': config.training.observation_type}
            kwargs.update(mask_params)
        if getattr(config.training, 'random_part_mask', False):
            mask_params = {'random_part_mask': True,
                           'mask_prob': config.training.mask_prob,
                           'apply_loss': config.training.apply_loss}
            kwargs.update(mask_params)
    ...
    def training_step(self, batch, batch_idx):
        wholebody_pose = torch.cat([batch['body_pose'], batch['left_hand_pose'], batch['right_hand_pose'],
                                    batch['jaw_pose'], batch['expression']], dim=1)
        mask = batch.get('mask', None)
        poses = self.normalize_fn(wholebody_pose, from_axis=True)
        # Forward pass and calculate loss
        loss_dict = self.train_step_fn(self.model, batch=poses, condition=None, mask=mask)
    ...
    def on_validation_epoch_start(self) -> None:
        ...
        self.compfn = DPoserComp(self.model, self.sde, self.config.training.continuous)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        eval_metrics, trajs, samples = self.process_validation_batch(poses, batch_idx == 0)
        self.all_samples.append(samples)

        # Store trajs of the first batch
        if batch_idx == 0:
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        body_out = self.body_model_eval(wholebody_params=body_pose)
    ...
    @torch.no_grad()
    def process_validation_batch(self, poses, gather_traj=False):
        ...
        '''     ******* task2 completion *******     '''
        mask, observation, mask_type = create_wholebody_mask(poses, part='lhand')
        ...
        evaler = Evaler(body_model=self.body_model_vis, part='lhand')
        ...
        trajs, samples = self.sampling_fn(
            self.model,
            observation=None, gather_traj=gather_traj,
        )  # [t, b, j*6], [b, j*6]
    ...
    def process_and_log_meshes(self, poses, bg_img, focal, princpt, tag_prefix):
        body_out = self.body_model_vis(wholebody_params=poses)
...
def main(args, config, try_resume):
    ...
    if config.dataset == 'wholebody':
        ...
        data_module = SmplxMixedDataModule(config, args)
    else:
        raise NotImplementedError(f'Dataset {config.dataset} not supported yet!')
    data_module.setup(stage='fit')
    model = DPoserTrainer(config, args.bodymodel_path, data_path, POSES_LIST,
