...
from lib.utils.misc import lerp, gaussian_smoothing
...
def parse_args(argv):
    ...
    parser.add_argument('--mode', type=str, default='optim', choices=['optim', 'repaint'],
                        help='note repaint mode is only for comparison, strongly NOT recommended')
    ...
    parser.add_argument('--post-smooth', type=bool, default=False, help='smooth the result')
...
class MotionDenoise(object):
    def __init__(self, config, args, diffusion_model, body_model, sds_weight=1.0,
                 out_path=None, debug=False, batch_size=1, pose_init=None):
        self.args = args
        self.debug = debug
    ...
    def DPoser_loss(self, x_0, vec_t, weighted=False, multi_denoise=False):
        # x_0: [B, j*6], vec_t: [B],
    ...
    def get_loss_weights(self):
        ...
        loss_weight = {'temp': lambda cst, it: 10. ** 1 * cst * (1 + it),
                       ...
                       'dposer': lambda cst, it: 10. ** -1 * cst * (1 + it) * self.sds_weight
                       }
        return loss_weight
    ...
    def optimize(self, joints3d, gt_poses=None, time_strategy='1', post_smooth=False, window_size=3, sigma=2,
        ...
        if vis:
            ...
            #                device=self.device)
            vis_body_skeletons(joints3d.detach().cpu().numpy(), output_path=os.path.join(self.out_path, 'renders'))
        ...
        if post_smooth:
            smooth_pose = gaussian_smoothing(smpl_init.body_pose.clone(), window_size=window_size, sigma=sigma)
            idx = [0, -1]
        ...
        pred_joints3d = smpl_init.Jtr[:, :22]
        if vis:
...
def main(args):
    ...
    if args.file_path is not None:
        ...
        batch_results = denoise(config, args, model, args.file_path, args.outpath_folder, std=args.noise_std, verbose=True)
        ...
        for seq in seqs:
            ...
            batch_results = denoise(config, args, model, gt_path, out_path, std=args.noise_std, verbose=False)

            # logging into csv
            with open(csv_file_path, mode='a', newline='') as csv_file:
                csv_writer = csv.writer(csv_file)
                avg_values = [np.mean(batch_results[key]) for key in batch_results.keys()]
