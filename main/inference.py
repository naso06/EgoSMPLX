...
def main():
    ...
    from utils.vis import render_mesh, save_obj, render_mesh_fisheye
    ...
    for frame in tqdm(range(start, end)):
        # img_path = os.path.join(args.img_path, f'img-04032021160435-{int(frame):06d}.jpg')
        img_path = os.path.join(args.img_path, f'img_{int(frame):06d}.jpg')
        ...
        for bbox_id in range(num_bbox):
            ...
            bbox_render = [int(mmdet_box[bbox_id][0]), int(mmdet_box[bbox_id][1]),
            ...
            img_crop, img2bb_trans, bb2img_trans = generate_patch_image(original_img, bbox, 1.0, 0.0, False, cfg.input_img_shape)
            img = transform(img_crop.astype(np.float32))/255
            img = img.cuda()[None,:,:,:]
            inputs = {'img': img}
            targets = {}
            meta_info = {}
            ...
            fisheye_cam = FishEyeCameraCalibrated("/home/cv1/works/SMPLer-X/main/fisheye.calibration_01_12_new.json")
            ...
            vis_img = render_mesh_fisheye(
                   ...
                   mesh,
                   smpl_x.face,
                   {'focal': focal, 'princpt': princpt},         # 기존 파라미터 (사용되지 않음)
                   fisheye_cam,
                   mesh_as_vertices=args.show_verts
               )
            if args.show_bbox:
