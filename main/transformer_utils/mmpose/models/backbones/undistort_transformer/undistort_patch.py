...
from ..utils.fisheye_camera.FishEyeCalibrated import FishEyeCameraCalibrated
from scipy.spatial.transform import Rotation as R
import open3d
import torch
from mmhuman3d.data.datasets.pipelines.loading import CropCircle, CropImage, ResizeImage
...
class UndistortPatch(nn.Module):
    def __init__(self,
        ...
        # # ======== 여기서 가로 방향 2개씩 버리기 ========
        # # patches_2d shape: (16*16, H_patch, W_patch, 2)
        # self.patches_2d = self.patches_2d.view(patch_num_vertical, patch_num_horizontal, *self.patches_2d.shape[1:])  # (16, 16, ...)
        # self.patches_2d = self.patches_2d[:, 2:-2, ...]  # (16, 12, ...)
        # self.patches_2d = self.patches_2d.contiguous().view(-1, *self.patches_2d.shape[2:])  # (16*12, ...)
        # # ============================================
    ...
    def forward(self, input_feature_map):
        ...
        input_feature_map = F.interpolate(input_feature_map, size=(256, 256), mode='bilinear', align_corners=True)
        ...
        rows, cols = 16, 16
        remove_left, remove_right = 2, 2
        selected_patch_indices = []
        for r in range(rows):
            for c in range(remove_left, cols - remove_right):
                selected_patch_indices.append(r * cols + c)
        selected_patch_indices = torch.tensor(selected_patch_indices, dtype=torch.long, device=sampled_patch.device)
      

        sampled_patch = sampled_patch[:, selected_patch_indices, ...]
