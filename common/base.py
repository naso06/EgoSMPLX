...
class Trainer(Base):
    ...
    def load_model(self, model, optimizer):
        ...
        if getattr(cfg, 'token_decoder_ckpt_path', None): 
            ...
            if missing:
                self.logger_info(f"  some missing (expected when 다른 파트 키 포함 안 함): {missing[:8]}{' ...' if len(missing)>8 else ''}")
            if unexpected:
                self.logger_info(f"  unexpected keys (확인 필요): {unexpected[:8]}{' ...' if len(unexpected)>8 else ''}")
            # set_trace()
    ...
    def _make_model(self):
        ...
        if self.use_lora:
            lora.mark_only_lora_as_trainable(model.module.encoder, bias='lora_only')  ## lora만 학습 가능하게 수정
