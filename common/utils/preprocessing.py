...
def process_human_model_output(human_model_param, cam_param, do_flip, img_shape, img2bb_trans, rot, human_model_type, joint_img=None):
    fisheye_camera = FishEyeCameraCalibrated("/home/cv1/works/SMPLer-X/main/fisheye.calibration_01_12.json")
    if human_model_type == 'smplx': 
        ...
        joint_cam_wo_ra = joint_cam.copy()
    ...
    if 'cam_rot' in human_model_param:
        ...
        R = human_model_param['cam_rot']
        t = human_model_param['cam_trans']
        J, _ = joint_cam.shape
        

        joint_cam_rotated = np.dot(joint_cam, R.T) + t  # (J, 3)
        joint_cam_torch = torch.from_numpy(joint_cam_rotated).float()
        # trans_torch = torch.from_numpy(trans).float()
        # joint_cam_translation =joint_cam_torch + trans
    ...
    #############
        ...
        J, _ = joint_cam_wo_ra.shape
        joint_cam_torch = torch.from_numpy(joint_cam_wo_ra).float()
        joint_cam_flat = (joint_cam_torch).view(-1, 3)
        joint_proj_flat = fisheye_camera.world2camera_pytorch(joint_cam_flat)  # (B*J, 2)
    ...
    if human_model_type == 'smplx':
        pose = pose.reshape(-1)
        lhand_pose = lhand_pose.reshape(-1)
        rhand_pose = rhand_pose.reshape(-1)
        expr = expr.numpy().reshape(-1)

        return joint_img, joint_cam, joint_trunc, pose, shape, expr, rotation_valid, coord_valid, expr_valid, mesh_cam_orig, lhand_pose, rhand_pose
    elif human_model_type == 'smpl':
        pose = pose.reshape(-1)
